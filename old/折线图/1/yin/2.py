...
def plot_degree_distribution(G, file_name, color, width=0.7):
    ...
    # 绘制曲线
    plt.plot(x, y, label=f"{file_name} Curve", color=color, alpha=0.8, linewidth=2)
...
# 添加标题
plt.title('Degree Distribution for Multiple Files (Bar Plot with Curve)', fontsize=14)

# 显示网格线
plt.grid(True, which='both', axis='y', linestyle='--', alpha=0.7)

# 保存图像
plt.savefig(f'{output_dir}/all_files_degree_distribution_with_curve.png', bbox_inches='tight')  # 使用 tight 确保保存时图形完整
plt.close()
